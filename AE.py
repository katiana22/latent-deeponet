...
num_train = 720 # for DeepONet training
...
n_train, n_test = x_y_train.shape[0], x_y_test.shape[0] # for autoencoder training
...
if args.method == 'CAE':
    ...
    ### WAE needs to be updated with x_y_data !!!!!
    y_train = y_train.reshape(y_train.shape[0]*nt, nx, ny)
    y_test = y_test.reshape(y_test.shape[0]*nt, nx, ny)
    
    y_train = np.expand_dims(y_train, axis=1) # Reshape (bs, 1, 28, 28)
    ...
    tensor_train = torch.Tensor(y_train) # transform to torch tensor
    tensor_test = torch.Tensor(y_test)
    tensor_all = torch.Tensor(y_all)
...
if args.method == 'WAE':        
    def calc_test_loss(criterion=nn.MSELoss(), args2=args2,
                       ...
                       encoder_filepath_list = ['save_weights/WAEgan_encoder-best_{}.pth'.format(args2['dataset'])],
                       decoder_filepath_list = ['save_weights/WAEgan_decoder-best_{}.pth'.format(args2['dataset'])]):
...
if args.method == 'vanilla-AE': 
    ...
    command = ['rm', '-r', 'save_weights']
    subprocess.run(command)

    command = ['mkdir', 'save_weights']
    subprocess.run(command)
    ...
    if args2['train']:
        ...
        for epoch in range(args2['epochs']):
            ...
            if args2['save']:
                save_path = './save_weights/WAEgan_{}-epoch_{}.pth'
        ...
        enc_checkpoint = torch.load('save_weights/WAEgan_encoder-best_{}.pth'.format(args2['dataset']))
        wae_encoder.load_state_dict(enc_checkpoint)

        dec_checkpoint = torch.load('save_weights/WAEgan_decoder-best_{}.pth'.format(args2['dataset']))
        wae_decoder.load_state_dict(dec_checkpoint)

        dec_checkpoint = torch.load('save_weights/WAEgan_discriminator-best_{}.pth'.format(args2['dataset']))
        discriminator.load_state_dict(dec_checkpoint)

    # Rename best weights
    command = ['mv', 'save_weights/WAEgan_encoder-epoch_{}.pth'.format(args2['epochs']-1), 'save_weights/WAEgan_encoder-best_custom.pth']
    subprocess.run(command)

    command = ['mv', 'save_weights/WAEgan_decoder-epoch_{}.pth'.format(args2['epochs']-1), 'save_weights/WAEgan_decoder-best_custom.pth']
    subprocess.run(command)

    command = ['mv', 'save_weights/WAEgan_discriminator-epoch_{}.pth'.format(args2['epochs']-1), 'save_weights/WAEgan_discriminator-best_custom.pth']
    subprocess.run(command)
...
if args.method == 'vanilla-AE' or args.method == 'MLAE' or args.method == 'CAE':
    ...
    # Split to train/test
    x_train_red = x_red[:num_train]
    x_test_red = x_red[num_train:]

    y_train_red = y_red[:num_train].reshape(num_train, nt, args.latent_dim)
    y_test_red = y_red[num_train:].reshape(num_test, nt, args.latent_dim)

    x_train = inputs[:num_train]
    x_test = inputs[num_train:]

    y_train = outputs[:num_train]
    y_test = outputs[num_train:] 
    ...
    decoded_data = decoder.forward(encoded_data).detach().numpy().reshape(n_test,nt*nx*ny)
    
    # Compute L2 error on test data
    reference_data = y_test.reshape(n_test,nt*nx*ny)
