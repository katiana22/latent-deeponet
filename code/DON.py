...
def error_metric(true, pred):
    #true - [samples, time steps, 28, 28]
    #pred - [samples, time steps, 28, 28]
    pred = np.reshape(pred, (-1, nt, nx, ny))
    num = np.abs(true - pred)**2 #[samples, time steps, 28, 28]
...
def show_error(don_model, ae_model, X_func, X_loc, pf_true, n_samples):
...
def main():
    ...
    if True:
        ...
        print('')
        print('Train Dataset')
        error_train = show_error(don_model, autoencoder, X_func_train[:n_samples], X_loc, pf_true_train, n_samples)

        print('Test Dataset')
        error_test = show_error(don_model, autoencoder, X_func_test[:n_samples], X_loc, pf_true_test, n_samples)

        if args.ood == 1:
            print('OOD Dataset')
            error_ood = show_error(don_model, autoencoder, X_func_ood[:num_ood], X_loc, pf_true_ood, num_ood)
        
        if args.noise == 1:
            print('Noisy Dataset')
            error_noisy = show_error(don_model, autoencoder, X_func_noisy[:num_noisy], X_loc, pf_true_noisy, num_noisy)
