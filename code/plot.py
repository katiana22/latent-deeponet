...
def errors_ae_don(latent_dim, method, reps, ood=0, noise=0):
    ...
    for j in range(len(method)):
        ...
        error_DON['Full\n DON'] = np.array([0.000921, 0.00091, 0.000905, 0.000893, 0.00088]) # Benchmark DeepONet!!
        error_DON['FNO-2D'] = np.array([0.00032, 0.00034, 0.000295, 0.00033, 0.00035]) # Benchmark 2D-FNO!!
        error_DON['FNO-3D'] = np.array([0.000141, 0.000155, 0.000188, 0.000176, 0.00016]) # Benchmark 3D-FNO!!
...
def violin_plots(method, latent_dim, pd_df_AE, pd_df_DON, pd_df_OOD, pd_df_NOISY, save):
    ...
    for i in range(len(method)):
        ...
        #plt.subplot(1,2,1)
        ax = sns.violinplot(data=pd_df_AE[i], linewidths=0.5, linecolor='k', palette='Set1')
        ...
        plt.tight_layout()
        if save==True:
            plt.savefig(dir_final_res + 'violin_plot_AE_{}_d_{}.png'.format(method[i],latent_dim), bbox_inches='tight', dpi=500)

        #plt.subplot(1,2,2)
        plt.rcParams.update({'font.size': 10})
        plt.figure(figsize=(6,3.4))
        ax = sns.violinplot(data=pd_df_DON[i], linewidths=0.5, linecolor='k', palette='Set1')
        plt.title('DeepONet')
        plt.ylabel('MSE')
        #plt.yscale('log')
        plt.grid(which='minor', alpha=1)
        plt.ticklabel_format(style='sci', axis='y', scilimits=(0,0))
        ax.axvline(3.5, color='k', linestyle='--', lw=1)
        plt.tight_layout()
        
        if save==True:
            plt.savefig(dir_final_res + 'violin_plot_latent_DON_{}_d_{}.png'.format(method[i],latent_dim), bbox_inches='tight', dpi=500)
